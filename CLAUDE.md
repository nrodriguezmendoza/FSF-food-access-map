...
Covers three Florida counties: Miami-Dade (FIPS `086`), Broward (`011`), Palm Beach (`099`).
...
2. `compute_index.py` — percentile-rank normalize indicators + weighted need score → `acs_with_index.csv`
...
- Tract geometry loads from the static `frontend/public/tracts_2022.geojson` (1497 tracts) and is joined **client-side** by `tract_id` GEOID against ACS values fetched from the API. Agency points load from static `frontend/public/agencies.geojson`.
...
- Tract IDs are 11-digit strings (state 2 + county 3 + tract 6, e.g. `12086013600`); the DB and `tracts_2022.geojson` GEOIDs match exactly (1497/1497). Preserve leading zeros — `.zfill(11)` when building keys from numeric sources.
